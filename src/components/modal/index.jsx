...
export default function Modal({ isShown, close }) {
  ...
  return (
    <div
      className={
        isShown ? [styles.modal, styles.showModal].join(" ") : styles.modal
      }
      ref={modalRef}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className={styles.modalCard} ref={modalCardRef}>
        <button ref={closeBtnRef} onClick={close} aria-label="close modal">
          &times;
        </button>
        <h3 id="modalHeading">Instructions</h3>
        <p>
          Click on a card to flip it over, then click a second card. If the
          cards match, they both remain face up. If they don't match, both cards
