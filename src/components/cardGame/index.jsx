...
export default function CardGame({
  ...
  useEffect(() => {
    if (cards) return;
    const cardList = [...cardTexts, ...cardTexts].map((card) => ({
      text: card,
      facedown: true,
      matched: false,
    }));
  ...
  function faceAllCardsDown() {
    setCards((cards) =>
      cards.map((c) => ({ ...c, matched: false, facedown: true }))
    );
  }
  ...
  function handleCardClick(clickedCardIndex) {
    ...
    const isFirstFlip = cards.filter((c) => !c.facedown).length % 2 === 0;
    ...
    const matchingFaceupCardIndex = cards.findIndex(
      (c) => !c.facedown && c.text === cards[clickedCardIndex].text
    );
